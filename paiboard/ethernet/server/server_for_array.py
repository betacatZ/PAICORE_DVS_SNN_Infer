...
uart_hex = "FFFFFFFFFFFFFFFE6405900032F8C8"
uart_hex_list = []
for i in range(0, len(uart_hex), 2):
    uart_hex_list.append(int("0x"+uart_hex[i:i + 2],16))


overlay = Overlay("./hw_files/PAICORE_Stream.bit")
xlnk = Xlnk()
xlnk.xlnk_reset()

rst_ip = overlay.ip_dict['pl_datapath/axi_gpio_0']
rst = AxiGPIO(rst_ip).channel1
mask = 0xffffffff
...
uart_list = []
uart_ADDRESS_list = [0x42C00000,0x42C10000,0x42C20000,0x42C30000]
for addr in uart_ADDRESS_list:
    uart_list.append(UartAXI(addr))
for uart_chip in uart_list:
    uart_chip.setupCtrlReg()

# uart_chip0 = UartAXI(uart_ADDRESS_list[0])
# uart_chip0.setupCtrlReg()

# rst.write(0xf, mask)
# sleep(0.02)
# rst.write(0x0, mask)
# sleep(0.02)
# send_num = uart_chip0.write_in_hex(uart_hex_list)
# sleep(0.02)
# print(uart_chip0.read_in_hex(send_num))
...
#                 print("Incompleted Package!")
    ...
    if work_mode == "SEND":
        ...
        tcpCliSock.sendall(send_buffer)
    elif work_mode == "CHIP RST":
        rst.write(0xf, mask)
        sleep(0.02)
        rst.write(0x0, mask)
        sleep(0.02)
    elif work_mode == "CHIP UART":
        uart_num = int(recv_data[2])
        print("Chip NO.",uart_num)
        
        recv_data = recv_data[3:18]
        uart_hex_list = []
        for i in range(0, 15):
            uart_hex_list.append(int(recv_data[i]))
#         send_num = uart_chip0.write_in_hex(uart_hex_list)
        send_num = uart_list[uart_num].write_in_hex(uart_hex_list)
        print(uart_list[uart_num].read_in_hex(send_num))
    elif work_mode == "QUIT":
        print('Wating...')
        tcpCliSock, addr = tcpSerSock.accept()
        print('Connected')
